...
from cogram.core.edges import EntityEdge
...
from cogram.prompts.dedupe_nodes import NodeDuplicate, NodeResolutions
from cogram.prompts.extract_nodes import (
    ExtractedEntities,
    ExtractedEntity,
    SummarizedEntities,
)
from cogram.search.search_filters import SearchFilters
from cogram.search.search_utils import node_similarity_search
from cogram.utils.datetime_utils import utc_now
from cogram.utils.maintenance.dedup_helpers import (
...
from cogram.utils.text_utils import (
    MAX_SUMMARY_CHARS,
...
async def resolve_extracted_nodes(
    ...
    # COGRAM PHASE 2: try embedding-cosine dedup before LLM escalation.
    # Most "obvious" duplicates (synonyms, paraphrases) resolve here for the
    # cost of one embedding call instead of a chat-completion dedup call.
    if state.unresolved_indices:
        embedding_candidate_nodes = _merge_candidate_nodes(
            [
                candidate
        ...
        await _resolve_with_embeddings(
            clients.embedder,
            extracted_nodes,
            _build_candidate_indexes(embedding_candidate_nodes),
            state,
        )
