...
if TYPE_CHECKING:
    from cogram.core.nodes import EntityNode
...
# COGRAM PHASE 2: embedding-cosine dedup threshold.
# Above this, two entities are treated as the same without consulting the LLM.
# 0.85 chosen conservatively for short entity names; can lower with validation.
_EMBEDDING_DUPLICATE_THRESHOLD = 0.85
...
def _cosine(a: list[float] | tuple[float, ...], b: list[float] | tuple[float, ...]) -> float:
    """Cosine similarity in pure Python; no numpy dependency at this hot path."""
...
async def _resolve_with_embeddings(
    ...
    indexes: DedupCandidateIndexes,
    state: DedupResolutionState,
    threshold: float = _EMBEDDING_DUPLICATE_THRESHOLD,
) -> None:
    """COGRAM PHASE 2: embedding-cosine dedup before LLM escalation.

    For each currently-unresolved index, generate a name embedding (cheap) and
    compare against candidate embeddings already loaded from the graph. If the
    best match exceeds threshold, mark as duplicate without calling the LLM.

    Mutates state in place: removes resolved indices from state.unresolved_indices.
    """
    if not state.unresolved_indices:
        return

    candidates_with_embeddings = [
        n for n in indexes.nodes_by_uuid.values() if n.name_embedding
    ]
    ...
    for idx in state.unresolved_indices:
        ...
        for candidate in candidates_with_embeddings:
            score = _cosine(node.name_embedding, candidate.name_embedding)
            if score > best_score:
                best_score = score
                best_candidate = candidate

        if best_candidate is not None and best_score >= threshold:
            promoted = _promote_resolved_node(node, best_candidate)
            state.resolved_nodes[idx] = promoted
            state.uuid_map[node.uuid] = promoted.uuid
            if promoted.uuid != node.uuid:
                state.duplicate_pairs.append((node, promoted))
        else:
            still_unresolved.append(idx)

    state.unresolved_indices = still_unresolved
...
__all__ = [
    ...
    '_EMBEDDING_DUPLICATE_THRESHOLD',
    '_cosine',
