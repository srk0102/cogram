    ...
    async def add_episode(
        ...
        with self.tracer.start_span('add_episode') as span:
            try:
                ...
                # ---- COGRAM HOOK ------------------------------------------------------
                # When COGRAM_FULL_PIPELINE=true, fire the cogram post-write pipeline:
                #   intent annotation on edges, narrative on entities, profile distill,
                #   knot synthesis, Redis cache invalidation. Runs as a background task
                #   so MCP latency stays bounded.
                # When unset, behaves like stock graphiti.
                # ----------------------------------------------------------------------
                import os as _os
                if _os.environ.get('COGRAM_FULL_PIPELINE', '').lower() == 'true':
                    try:
                        import asyncio as _asyncio
                        from cogram.pipeline.post_write import cogram_post_write
                        from cogram.core.config import Settings as _Settings
                        ...
                        logger.warning(f'cogram_post_write hook scheduling failed: {_hook_exc}')
...
# ---------------------------------------------------------------------------
# Backward compatibility: keep the original 'Graphiti' name importable
# (any code doing 'from cogram.main import Graphiti' or 'from cogram import
# Graphiti' continues to work after the class rename to 'Cogram').
# ---------------------------------------------------------------------------
Graphiti = Cogram
