...
class LLMClient(ABC):
    ...
    async def _generate_response_with_retry(
        ...
        # ---- COGRAM ENGRAM CACHE ---------------------------------------------
        # Check Postgres engram.patterns before hitting the LLM. Cache hit returns
        # the stored response immediately — zero LLM cost. Cache miss proceeds with
        # the actual LLM call, then writes the result back. Only active when
        # POSTGRES_DSN is set; falls through transparently otherwise.
        # ----------------------------------------------------------------------
        _engram_put = None
        namespaced_key: str | None = None
        try:
            from cogram.llm_client.engram import _pg_get, _pg_put as _engram_put
            cache_key = self._get_cache_key(messages)
            namespaced_key = f'llm:{self.model}:{cache_key}'
            cached = await _pg_get(namespaced_key)
            if cached is not None:
                cached_resp, _conf = cached
        ...
        try:
            response = await self._generate_response(messages, response_model, max_tokens, model_size)
        except (httpx.HTTPStatusError, RateLimitError) as e:
            raise e

        try:
            if _engram_put is not None and namespaced_key is not None and isinstance(response, dict):
                await _engram_put(namespaced_key, kind='llm_response', response=response, confidence=1.0)
        except Exception:
            pass
